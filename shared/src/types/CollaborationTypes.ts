...
const permissions = (
  overrides: Partial<CollaborationTierPermissions> = {},
...
export const DEFAULT_COLLABORATION_TIERS: CollaborationAccessTier[] = [
  {
    id: 'viewer',
    name: 'Viewer',
    description: 'Read-only access to the shared session.',
    builtin: 'viewer',
    color: '#60a5fa',
    allowedModels: [],
    reasoningByModel: {},
  ...
  },
  {
    id: 'collaborator',
    name: 'Collaborator',
    description: 'Can propose work; the host approves execution. Can use shared providers.',
    builtin: 'collaborator',
    color: '#f59e0b',
    allowedModels: [],
    reasoningByModel: {},
    permissions: permissions({ submitPrompts: true, useRemoteProviders: true }),
  },
  {
    id: 'yolo',
    name: 'YOLO',
    description:
      'Unrestricted prompt, model, tool, and filesystem access. Use only for trusted people.',
    builtin: 'yolo',
    color: '#ef4444',
    allowedModels: ['*'],
    reasoningByModel: {},
  ...
  // raises this per-tier if they trust the guest with their machine.
  {
    ...
    name: 'Model Access',
    description:
      'Use the host’s shared models in their own workspace. CLI models can read+edit their files but cannot run shell commands on the host.',
    builtin: 'models',
    color: '#a78bfa',
    allowedModels: ['*'],
    reasoningByModel: {},
...
export const DEFAULT_COLLABORATION_POLICY: CollaborationPolicy = {
  ...
  accessTiers: DEFAULT_COLLABORATION_TIERS,
  allowedModels: [],
  allowPrompts: true,
