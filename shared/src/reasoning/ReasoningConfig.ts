...
// Provider list that doesn't support reasoning (static fallback only).
// Providers enriched via models.dev (applyModelsDevMetadata) are NOT listed
// here — their reasoning levels come from live per-model metadata, and a
// contradictory null rule here would suppress the picker if enrichment
// regressed. See PROVIDER_KEY in backend/src/providers/models-dev.ts.
