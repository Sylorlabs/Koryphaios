<script lang="ts">
  ...
  const readPathsText = $derived(selectedTier?.permissions.readPaths.join('\n') ?? '');
  const writePathsText = $derived(selectedTier?.permissions.writePaths.join('\n') ?? '');
  const commandAllowlistText = $derived((selectedTier?.permissions.commandAllowlist ?? []).join('\n'));
  const commandBlocklistText = $derived((selectedTier?.permissions.commandBlocklist ?? []).join('\n'));
  const accessTierOptions = $derived((policy?.accessTiers ?? []).map(tier => ({ value: tier.id, label: tier.name })));
...
{#if policy && selectedTier}
  <div class="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-6 space-y-6">
    ...
    <div class="rounded-2xl border border-[var(--color-accent)]/25 p-4" style="background:color-mix(in srgb,var(--color-accent) 6%,var(--color-surface-1))">
      ...
      <div class="mt-3 flex flex-wrap gap-2">{#each effectiveSummary as item (item)}<span class="rounded-lg border px-2.5 py-1.5 text-[10px]" style="border-color:var(--color-border);background:var(--color-surface-1);color:var(--color-text-secondary)">{item}</span>{/each}</div>
    </div>
    ...
    {#if advanced}<div class="grid gap-5 lg:grid-cols-[1fr_1.4fr]">
                ...
                <div class="rounded-lg p-2 hover:bg-[var(--color-surface-3)]">
                  ...
                  {#if (selectedTier.allowedModels.includes('*') || selectedTier.allowedModels.includes(item.id)) && item.reasoningLevels.length}
                    <div class="ml-5 mt-2 flex flex-wrap gap-1">
                      {#each item.reasoningLevels as level (level)}
      ...
      <div class="space-y-4">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="text-[10px] font-bold uppercase text-[var(--color-text-muted)]">Readable paths<textarea value={readPathsText} onchange={(e) => updatePermission('readPaths', e.currentTarget.value.split('\n').map(v => v.trim()).filter(Boolean))} placeholder="src/**&#10;docs/**" rows="5" class="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-3 font-mono text-xs normal-case"></textarea></label>
          <label class="text-[10px] font-bold uppercase text-[var(--color-text-muted)]">Writable paths<textarea value={writePathsText} onchange={(e) => updatePermission('writePaths', e.currentTarget.value.split('\n').map(v => v.trim()).filter(Boolean))} placeholder="src/components/**" rows="5" class="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-3 font-mono text-xs normal-case"></textarea></label>
        </div>
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="text-[10px] font-bold uppercase text-[var(--color-text-muted)]">Command allowlist<span class="mt-1 block font-normal normal-case">Executable names or wildcards. Empty allows the normal Koryphaios sandbox.</span><textarea value={commandAllowlistText} onchange={(e) => updatePermission('commandAllowlist', e.currentTarget.value.split('\n').map(v => v.trim()).filter(Boolean))} placeholder="git&#10;bun&#10;npm&#10;cargo" rows="5" class="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-3 font-mono text-xs normal-case"></textarea></label>
          <label class="text-[10px] font-bold uppercase text-[var(--color-text-muted)]">Command blocklist<span class="mt-1 block font-normal normal-case">Always wins over the allowlist.</span><textarea value={commandBlocklistText} onchange={(e) => updatePermission('commandBlocklist', e.currentTarget.value.split('\n').map(v => v.trim()).filter(Boolean))} placeholder="rm&#10;sudo&#10;curl&#10;ssh" rows="5" class="mt-2 w-full rounded-xl border border-red-500/20 bg-[var(--color-surface-1)] p-3 font-mono text-xs normal-case"></textarea></label>
        </div>
      </div>
    </div>{/if}
