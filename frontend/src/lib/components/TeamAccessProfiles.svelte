...
{#if policy && selectedTier}
  <div class="rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface-2)] p-6 space-y-6">
    ...
    <div class="grid gap-4 border-t border-[var(--color-border)] pt-5 md:grid-cols-2">
      <label class="text-xs font-bold">Code join behavior<select value={policy.joinMode} onchange={(e) => collaborationStore.updatePolicy({ joinMode: e.currentTarget.value as 'approval'|'auto' })} class="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-3 text-xs"><option value="approval">Host must approve each person</option><option value="auto">Automatically admit with default profile</option></select></label>
      <label class="text-xs font-bold">Default profile for join code<select value={policy.defaultTierId} onchange={(e) => collaborationStore.updatePolicy({ defaultTierId: e.currentTarget.value })} class="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-1)] p-3 text-xs">{#each policy.accessTiers as tier}<option value={tier.id}>{tier.name}</option>{/each}</select></label>
    </div>

    {#if collaborationStore.pendingJoins.length}<div class="space-y-2 border-t border-[var(--color-border)] pt-5"><div class="text-xs font-bold text-amber-400">Waiting for host approval</div>{#each collaborationStore.pendingJoins as join (join.guestId)}<div class="flex flex-wrap items-center gap-3 rounded-xl bg-[var(--color-surface-1)] p-3"><span class="min-w-32 flex-1 text-xs font-bold">{join.name}</span><select bind:value={joinAssignments[join.guestId]} class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)] p-2 text-xs">{#each policy.accessTiers as tier}<option value={tier.id}>{tier.name}</option>{/each}</select><button onclick={() => collaborationStore.decideJoin(join.guestId, true, joinAssignments[join.guestId] || policy.defaultTierId)} class="rounded-lg bg-emerald-500/10 px-3 py-2 text-xs font-bold text-emerald-400">Admit</button><button onclick={() => collaborationStore.decideJoin(join.guestId, false)} class="rounded-lg bg-red-500/10 px-3 py-2 text-xs font-bold text-red-400">Reject</button></div>{/each}</div>{/if}
    {#if collaborationStore.participants.length}<div class="space-y-2 border-t border-[var(--color-border)] pt-5"><div class="text-xs font-bold">Connected people</div>{#each collaborationStore.participants as person (person.guestId)}<div class="flex items-center gap-3 rounded-xl bg-[var(--color-surface-1)] p-3"><span class="flex-1 text-xs font-bold">{person.name}</span><select value={person.tierId} onchange={(e) => collaborationStore.assignTier(person.guestId, e.currentTarget.value)} class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-2)] p-2 text-xs">{#each policy.accessTiers as tier}<option value={tier.id}>{tier.name}</option>{/each}</select></div>{/each}</div>{/if}
  </div>
{/if}
