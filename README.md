...
**CLI integrations depend on the installed CLI and local profile.** Koryphaios can detect an executable or login file without claiming that the account is authenticated. Those signals remain explicitly detected and unverified unless a supported read-only account probe succeeds; for file-only harnesses, definitive access is known only when the CLI actually runs. Results can vary with CLI version, profile layout, provider entitlement, and sandbox support. Kilo execution, the undocumented Freebuff SDK path, and approval-requiring Jules mutation are deliberately unavailable in this build rather than reported as working.
