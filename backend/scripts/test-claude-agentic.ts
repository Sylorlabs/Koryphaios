...
import type { ProviderEvent, StreamRequest } from '../src/providers/types';
...
const p = new ClaudeCodeProvider({ name: 'claude', disabled: false } satisfies ProviderConfig);
const fileEdits: ProviderEvent[] = [];
const tools: ProviderEvent[] = [];
let text = '';
...
for await (const e of p.streamResponse({
  ...
  messages: [
    {
      role: 'user',
      content:
        'Create a file named pwagent.txt containing exactly the word mango. Then use Edit to change mango to kiwi.',
    },
  ],
  ...
  if (e.type === 'file_edit') {
    ...
    console.log(
      ...
      'content=',
      JSON.stringify(e.fileContent),
      'old=',
      JSON.stringify(e.fileOldContent),
    );
...
console.log(
  'pwagent.txt exists:',
  existsSync(`${DIR}/pwagent.txt`),
  '| contents:',
  existsSync(`${DIR}/pwagent.txt`)
    ? JSON.stringify(readFileSync(`${DIR}/pwagent.txt`, 'utf-8'))
    : 'N/A',
);
