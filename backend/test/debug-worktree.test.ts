...
const TEST_DIR = "/tmp/worktree-real-test";

test("debug worktree spawn", () => {
  // Setup fresh repo
  ...
  console.log("Can spawn:", workspace.canSpawn());
  console.log("Status:", workspace.getStatus());
  
  // Try to spawn
  const worktree = workspace.spawn("test-1", "Test task", "agent-1");
  console.log("Spawn result:", worktree);
