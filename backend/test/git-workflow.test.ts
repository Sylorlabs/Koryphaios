...
// NOTE: These tests require git to be installed and available in PATH
...
function isGitAvailable(): boolean {
  const result = spawnSync(["git", "--version"], { stdout: "pipe", stderr: "pipe" });
...
describeIfGit("Git Workflow Integration Tests", () => {
  ...
  describe("GitManager", () => {
    ...
    test("should get current branch", async () => {
      ...
      expect(branch).toBe("main"); // or "master" depending on git version
    });

    test("should get current hash", () => {
      const git = new GitManager(TEST_DIR);
    ...
    test("should rollback to previous hash", async () => {
      ...
      // Rollback
      const rolledBack = git.rollback(originalHash!);
  ...
  describe("AutoCommitService", () => {
    ...
    test("should generate branch name from task", async () => {
      ...
      expect(existsSync(join(TEST_DIR, "auto-test.txt"))).toBe(true);
    });
  ...
  describe("ShadowLogger", () => {
    ...
    test("should recover to ghost state", async () => {
      ...
      const result = shadowLogger.recover(targetHash);
      expect(result.success).toBe(true);
  ...
  describe("SnapshotManager", () => {
    ...
    test("should create snapshot", async () => {
      ...
      await snapshotManager.createSnapshot("test-session", "latest", ["snapshot-test.txt"], TEST_DIR);
