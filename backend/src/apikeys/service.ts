  ...
  async revoke(userId: string, id: string): Promise<boolean> {
    ...
    if (rows.length > 0) {
      await this.audit
        ...
        .catch((err: unknown) => {
          // Audit logging is non-critical — revocation still succeeds.
          serverLog.debug({ err: err instanceof Error ? err.message : String(err), action: 'api_key_revoke' }, 'API key revoke audit log failed (non-critical)');
        });
    }
