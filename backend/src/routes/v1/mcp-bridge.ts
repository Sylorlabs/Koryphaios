...
const CRITIC_MCP_TOOLS = new Set([
  'read_file',
  'grep',
  'glob',
  'ls',
...
const MANAGER_ONLY_MCP_TOOLS = new Set([
  'delegate_to_worker',
  'delegate_to_jules',
  'create_goal',
...
export const mcpBridgeRoutes = new Elysia({ prefix: '/api/v1/mcp-bridge' })
  ...
  .post(
    ...
    async ({ request, body, set }) => {
      const session: SessionToken | null = validateLocalBearerToken(request.headers.get('authorization'));
      ...
      const normalizedRole = role === 'critic' || role === 'worker' ? role : 'manager';
      if (!localAuth.hasPermission(session, `mcp:${sessionId}:${normalizedRole}`)) {
        set.status = 403;
        return { ok: false, error: 'MCP bridge token is not valid for this session and role.' };
      }
      if (
        (normalizedRole === 'critic' && !CRITIC_MCP_TOOLS.has(toolName)) ||
        (normalizedRole === 'worker' && MANAGER_ONLY_MCP_TOOLS.has(toolName))
      ) {
        set.status = 403;
      ...
      try {
        ...
        const sessionRow = await sessions.get(sessionId);
        if (!sessionRow) {
          set.status = 404;
          return { ok: false, error: `Session ${sessionId} not found` };
        }
        const resolvedWorkingDirectory = workingDirectory ?? sessionRow.workingDirectory;
        if (!resolvedWorkingDirectory) {
          set.status = 400;
          return {
            ok: false,
            error: 'workingDirectory is required when the session has no working directory set.',
          };
        }
        const ctx: ToolContext = {
          ...
          preflightFileChange: (proposal) =>
            kory.enforceAutonomyLimits?.(sessionId, proposal) ?? Promise.resolve({ allowed: true }),
        };
  ...
  .post(
    ...
    async ({ request, body, set }) => {
      ...
      try {
        ...
        const sessionRow = await sessions.get(session_id);
        if (!sessionRow) {
        ...
        if (tool_name === 'run_subagent' || tool_name === 'Task' || tool_name === 'Agent') {
          return {
            decision: 'block' as const,
            reason:
              'Koryphaios owns orchestration. Use kory__delegate_to_worker to delegate tasks.',
          };
        }
  ...
  .post(
    ...
    async ({ request, body, set }) => {
      ...
      try {
        const { kory, sessions } = getContext();
        const sessionRow = await sessions.get(session_id);
        if (!sessionRow) return { ok: true, additionalContext: '' };
      ...
      try {
        ...
        if (mayStop === false) {
          return {
            decision: 'block' as const,
            reason:
              'The Koryphaios critic has not verified the work as complete. Continue working.',
          };
        }
        return { decision: 'approve' as const };
      } catch (error) {
        return {
          decision: 'block' as const,
          reason: `Koryphaios could not establish completion verification: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    },
    { body: t.Object({ session_id: t.String() }) },
