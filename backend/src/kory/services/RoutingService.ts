// RoutingService - Handles provider routing, model resolution, and fallback chains
// Extracted from KoryManager to separate concerns
// Merged from routing-service.ts (root) and services/RoutingService.ts
...
import { DOMAIN, AGENT } from "../../constants";
...
export class RoutingService {
  constructor(
    ...
    private config: Pick<KoryphaiosConfig, "fallbacks" | "assignments" | "agents">
  ) {}

  /**
   * Build fallback chain for a starting model
  ...
  /**
   * Resolve active routing for a domain.
   * Alias for resolveRouting for backward compatibility.
   *
   * @param preferredModel - User-specified model (optional)
   * @param domain - Worker domain
   * @param avoidLegacy - Whether to avoid legacy/deprecated models
  ...
  resolveActiveRouting(
    ...
    return this.resolveRouting(preferredModel, domain, avoidLegacy);
  }

  /**
   * Resolve provider with fallback to first available
   *
   * @param preferredModel - User-specified model (optional)
   * @param domain - Worker domain
   * @param avoidLegacy - Whether to avoid legacy models
   * @returns Object containing routing decision and resolved provider
   */
  async resolveProviderWithFallback(
    preferredModel?: string,
    domain: WorkerDomain = "general",
    avoidLegacy = false
  ): Promise<{ routing: RoutingDecision; provider: Provider | undefined }> {
    const routing = this.resolveRouting(preferredModel, domain, avoidLegacy);
    ...
    if (!provider && (!preferredModel || preferredModel === "auto")) {
      ...
      if (fallback) {
        const fallbackRouting = { model: fallback.model, provider: fallback.provider };
        provider = await this.providers.resolveProvider(fallbackRouting.model, fallbackRouting.provider);
        if (provider) {
          return { routing: fallbackRouting, provider };
        }
      }
    }
  ...
  /**
   * Get reasoning level for workers
   *
   * @returns The configured reasoning level for workers
   */
  ...
  /**
   * Classify domain based on message content using keyword scoring.
   *
  ...
  /**
   * Classify a message into a worker domain using keyword scoring.
   * Alias for classifyDomain for backward compatibility.
   *
   * @param message - The user message to classify
   * @returns The best-matching worker domain
   */
  classifyDomainLLM(message: string): WorkerDomain {
    return this.classifyDomain(message);
  }

  /**
   * Check if message requires system access
   *
   * @param message - The message to check
   * @returns true if system access is required
   */
  requiresSystemAccess(message: string): boolean {
    return ["install", "sudo", "apt"].some((k) => message.toLowerCase().includes(k));
  }
  ...
  async extractAllowedPaths(sessionId: string, plan: string, preferredModel?: string): Promise<string[]> {
    const routing = this.resolveRouting(preferredModel, "general", true);
    ...
    try {
      ...
      koryLog.debug({ error: err instanceof Error ? err.message : String(err) }, "Failed to extract allowed paths");
      return [];
    }
  }
