...
export class ConflictResolutionService {
  ...
  async resolveConflicts(conflicts?: Conflict[]): Promise<ResolutionResult> {
    ...
    const autoResolvable = detectedConflicts.filter(c => 
      this.shouldAutoResolve(c.severity)
    );
    const needsEscalation = detectedConflicts.filter(c => 
      !this.shouldAutoResolve(c.severity)
    );
    ...
    const message = `Resolved ${resolved}/${autoResolvable.length} auto-resolvable conflicts. ` +
  ...
  async arbitrateWithCritic(conflict: Conflict): Promise<{ 
    ...
    try {
      const provider = this.providers.findProviderForModel(this.config.criticModel || "claude-3-7-sonnet");
      if (!provider) {
      ...
      const stream = provider.streamResponse({
        model: this.config.criticModel,
        messages: [
          {
            role: "system",
            content: `You are an expert code reviewer and conflict resolver. Your task is to analyze conflicting changes from multiple AI agents and determine the best resolution.
...
- content: the resolved code (if merge or selection)
- reasoning: explanation of your decision`,
  ...
  private shouldAutoResolve(severity: Conflict["severity"]): boolean {
    ...
    const levels: Record<Conflict["severity"] | "none", number> = { low: 1, medium: 2, high: 3, critical: 4, none: 5 };
  ...
  private buildArbitrationPrompt(conflict: Conflict): string {
    const changesText = conflict.changes.map((change, idx) => `
...
Analyze these conflicting versions and provide your resolution decision.`;
  ...
  private extractHunks(base: string, modified: string): Array<{
    startLine: number;
    endLine: number;
    content: string;
  }> {
  ...
  private isHunkOverlapping(content: string, hunk: { startLine: number; endLine: number }): boolean {
