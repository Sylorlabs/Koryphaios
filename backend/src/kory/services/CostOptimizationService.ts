...
export class CostOptimizationService {
  ...
  analyzeTaskProfile(prompt: string): TaskProfile {
    ...
    if (lowerPrompt.includes("classify") || lowerPrompt.includes("category")) {
      ...
      type = "coding";
    } else if (lowerPrompt.includes("explain") || lowerPrompt.includes("why") || lowerPrompt.includes("reason")) {
      type = "reasoning";
    }
    ...
    const reasoningIndicators = [
      "explain", "why", "how", "reason", "analyze", "compare", "evaluate",
      "step by step", "think through", "logic", "complex", "difficult",
    ];
  ...
  storeInCache(
    ...
    const cost = modelInfo 
      ? (tokensIn / 1000 * modelInfo.costPer1KInput) + (tokensOut / 1000 * modelInfo.costPer1KOutput)
      : 0;
  ...
  clearCache(tags?: string[]): void {
    ...
    koryLog.info({ tags: tags || "all" }, "Cache cleared");
  }
  ...
  async deduplicateRequest<T>(
    ...
    if (existing) {
      koryLog.debug({ key }, "Deduplicating request");
  ...
  /**
   * Record routing savings
   */
  recordRoutingSavings(savedCost: number): void {
    const today = this.getOrCreateTodayMetrics();
    today.savingsFromRouting += savedCost;
  }
  ...
  getAnalytics(days = 30): {
    ...
    const totalSavings = recentMetrics.reduce(
      (sum, m) => sum + m.savingsFromCache + m.savingsFromRouting, 0
    );
  ...
  private findModel(modelName: string): ModelCapability | undefined {
    return this.models.find(m => 
      m.model === modelName || m.model.includes(modelName)
    );
  }
  ...
  private checkBudgetAlerts(dateKey: string, monthKey: string): void {
    ...
    if (!this.alertSent) {
      if (status.dailyUsed / this.config.budget.dailyLimit > threshold ||
