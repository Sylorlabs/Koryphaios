...
type ApprovalEventType = "permission.request" | "permission.response";
...
export class HumanInTheLoopService {
  ...
  requiresApproval(operation: Omit<Operation, "id" | "requestedAt">): boolean {
    ...
    if (policy.costThreshold && operation.estimatedCost && 
  ...
  private emitApprovalRequest(request: ApprovalRequest): void {
    const message: WSMessage = {
      ...
      payload: {
        ...
        estimatedCost: request.operation.estimatedCost,
        expiresAt: request.expiresAt,
        timeoutMs: request.timeoutMs,
      },
    };
    wsBroker.publish(request.operation.sessionId, message as any);
  }

  private emitDecisionUpdate(decision: ApprovalDecision): void {
    const message: WSMessage = {
      type: "permission.response",
      sessionId: "", // Would need to look up from operation
      payload: {
        operationId: decision.operationId,
        approved: decision.approved,
        reason: decision.reason,
        decidedBy: decision.decidedBy,
        timeoutOccurred: decision.timeoutOccurred,
      },
    };
    wsBroker.publish("broadcast", message as any);
  }
