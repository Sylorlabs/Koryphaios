...
export class AgentOpsService {
  ...
  rollbackToVersion(name: string, version: string): PromptVersion | undefined {
    const target = Array.from(this.prompts.values())
  ...
  compareVersions(idA: string, idB: string): {
    ...
    if (promptA.metadata.temperature !== promptB.metadata.temperature) {
      differences.push(`Temperature changed: ${promptA.metadata.temperature} -> ${promptB.metadata.temperature}`);
    }
    ...
    const similarity = maxLen > 0 
      ? 1 - (this.levenshteinDistance(promptA.content, promptB.content) / maxLen)
      : 1;
   ...
   */
  createDataset(
    dataset: Omit<EvaluationDataset, "id" | "createdAt">
  ): EvaluationDataset {
  ...
  compareEvaluations(
    ...
    if (!resultsA || !resultsB) {
      return { improvement: 0, significant: false, details: { accuracyDelta: 0, latencyDelta: 0, costDelta: 0 } };
    }
    ...
    const improvement = (accuracyDelta + (latencyDelta > 0 ? 0.1 : 0) + (costDelta > 0 ? 0.1 : 0)) / 3;
