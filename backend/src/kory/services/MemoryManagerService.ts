...
export class MemoryManagerService {
   ...
   */
  async store(entry: Omit<MemoryEntry, "id" | "timestamp" | "accessCount" | "lastAccessed">): Promise<MemoryEntry> {
    ...
    if (query.content && this.config.vector.enabled) {
      ...
      for (const semanticResult of semanticResults) {
        ...
        if (existing) {
          ...
          existing.relevanceScore = Math.max(existing.relevanceScore, semanticResult.relevanceScore);
  ...
  /**
   * Retrieve relevant context from all tiers for a query
   */
  async getRelevantContext(query: string, sessionId: string): Promise<{
    ...
    for (const result of results) {
      if (this.shortTermMemory.has(result.entry.id)) {
        shortTerm.push(result.entry);
      } else if (this.longTermMemory.has(result.entry.id)) {
        longTerm.push(result.entry);
      }
      if (result.semanticScore && result.semanticScore > 0.7) {
        semantic.push(result.entry);
      }
    }

    // Get relevant summary if available
    const summary = this.summaries.find(s => s.sessionId === sessionId);

    return { shortTerm, longTerm, semantic, summary };
  }
  ...
  private async pruneContext(sessionId: string): Promise<void> {
    ...
    const targetTokens = context.maxTokens * 0.8; // Prune to 80% capacity
  ...
  /**
   * Compress long contexts using summarization
   */
  async compressContext(sessionId: string): Promise<string> {
    const context = this.sessionContexts.get(sessionId);
    if (!context || context.totalTokens < this.config.compression.thresholdTokens) {
      return this.getContextWindow(sessionId);
    }
    ...
    const windows = this.groupByTimeWindow(context.entries, 5 * 60 * 1000); // 5 min windows
    const summaries: string[] = [];

    for (const window of windows) {
      const windowText = window.map(e => this.formatEntry(e)).join("\n");
      const summary = await this.summarize(windowText);
   ...
   */
  async createSummary(sessionId: string): Promise<ConversationSummary> {
    const context = this.sessionContexts.get(sessionId);
    if (!context || context.entries.length === 0) {
      throw new Error("No context to summarize");
    }

    const fullText = context.entries.map(e => this.formatEntry(e)).join("\n");
    
    const summary: ConversationSummary = {
      id: `summary-${Date.now()}`,
      sessionId,
      startTime: context.entries[0]?.timestamp || Date.now(),
      endTime: context.entries[context.entries.length - 1]?.timestamp || Date.now(),
      summary: await this.summarize(fullText),
      keyPoints: await this.extractKeyPoints(fullText),
      entities: this.extractEntities(context.entries),
      decisions: this.extractDecisions(context.entries),
      tokenCount: Math.floor(fullText.length / 4), // Rough token estimate
    };
    ...
    await this.store({
      ...
      tokens: Math.floor(summary.summary.length / 4),
      sessionId,
      agentId: "system",
      importance: 0.9,
      metadata: {
        type: "summary",
        tags: ["summary", sessionId],
      },
    });
  ...
  cleanup(): void {
    ...
    // Clean vector memory
    this.vectorMemory = this.vectorMemory.filter(
      v => now - v.metadata.timestamp < ttl * 2
    );
  ...
  private async pruneWithSummarization(
    ...
    const summary = await this.summarize(textToSummarize);
  ...
  private groupByTimeWindow(entries: MemoryEntry[], windowMs: number): MemoryEntry[][] {
    const windows: MemoryEntry[][] = [];
    let currentWindow: MemoryEntry[] = [];
    let windowStart = 0;

    for (const entry of entries.sort((a, b) => a.timestamp - b.timestamp)) {
      if (currentWindow.length === 0 || entry.timestamp - windowStart <= windowMs) {
        currentWindow.push(entry);
        if (currentWindow.length === 1) windowStart = entry.timestamp;
      } else {
  ...
  private async summarize(text: string): Promise<string> {
    // Simplified - in reality, call LLM to summarize
    return `[Summary of ${Math.floor(text.length / 4)} tokens: ${text.slice(0, 100)}...]`;
  }

  private async extractKeyPoints(text: string): Promise<string[]> {
    // Simplified - in reality, use LLM to extract key points
    return ["Key point extraction not implemented"];
  }

  private extractEntities(entries: MemoryEntry[]): string[] {
    const entities = new Set<string>();
    for (const entry of entries) {
      if (entry.metadata.entities) {
        entry.metadata.entities.forEach(e => entities.add(e));
      }
    }
    return Array.from(entities);
  }

  private extractDecisions(entries: MemoryEntry[]): string[] {
    return entries
      .filter(e => e.metadata.type === "thought" && e.content.toLowerCase().includes("decide"))
      .map(e => e.content);
  }
