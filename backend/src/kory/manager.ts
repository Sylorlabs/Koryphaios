...
import { existsSync, mkdirSync, readFileSync, writeFileSync, rmSync, createWriteStream, WriteStream } from "fs";
...
function initTraceLog(workingDir: string) {
  ...
  traceStream.on('error', (e) => {
    koryLog.warn({ e }, "Trace stream error");
  });
