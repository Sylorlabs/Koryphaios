...
import { ToolRegistry, BashTool, ReadFileTool, WriteFileTool, GrepTool } from '../../tools';
...
describe('KoryManager State', () => {
  ...
  test('should cleanup session resources', async () => {
    ...
    // After cleanup, the session should no longer be reported as running
    expect(kory.isSessionRunning(session.id)).toBe(false);
    // Memory stats should reflect no active workers for the cleaned-up session
    const stats = kory.getMemoryStats();
    expect(stats.activeWorkers ?? 0).toBe(0);
  });
});
...
describe('YOLO Mode', () => {
  test('should toggle YOLO mode', () => {
    ...
    expect(kory.getIsYoloMode()).toBe(true);
    kory.setYoloMode(false);
    expect(kory.getIsYoloMode()).toBe(false);
  });
});
...
describe('Circuit Breaker', () => {
  test('should create circuit breaker for providers', async () => {
    const { getCircuitBreaker } = await import('../../resilience/circuit-breaker');
...
describe('Bounded Cache', () => {
  test('should create bounded cache with TTL', async () => {
    const { BoundedCache } = await import('../../resilience/bounded-cache');
