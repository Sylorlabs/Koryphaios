...
describe('MCP stdio framing and lifecycle', () => {
  ...
  test('rejects immediately and clears waiters when stdio closes during a write', async () => {
    const client = new MCPClient({ name: 'write-failure', transport: 'stdio' });
    const internals = testable(client) as TestableMCPClient & {
    ...
    const processHandle = {
      ...
      stdin: {
        write() {
          throw new Error('synthetic closed pipe');
        },
        flush() {},
      },
    };
    ...
    await expect(internals.request('tools/call', { name: 'fixture' })).rejects.toThrow(
      'MCP stdio request write failed',
    );
    expect(internals.pending.size).toBe(0);
    expect(internals.process).toBeUndefined();
    expect(processHandle.kills).toBe(1);
  ...
  test.skipIf(!BUN_STDIO_PIPES_AVAILABLE)(
    'rejects a pending tool call immediately when the child exits',
    async () => {
  ...
  test.skipIf(!BUN_STDIO_PIPES_AVAILABLE)(
    'shutdown rejects pending work instead of silently clearing it',
    async () => {
  ...
  test.skipIf(!BUN_STDIO_PIPES_AVAILABLE)(
    'MCP manager refuses same-name servers from a different project scope',
    async () => {
