        ...
        models: [
          {
            id: 'gpt-5.6-sol',
            name: 'GPT 5.6 Sol',
            provider: 'remote-codex' as never,
...
describe('applyRemoteFileEdit (shared CLI edit apply)', () => {
  let root: string;
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), 'kory-rp-'));
  });
  afterEach(async () => {
  ...
  test('edit splices the snippet and PRESERVES the rest of the file (regression)', async () => {
    ...
    expect(
      await applyRemoteFileEdit(root, {
        type: 'file_edit',
        filePath: 'a.txt',
        fileOldContent: 'line 2 TARGET here',
        fileContent: 'line 2 REPLACED',
        fileOperation: 'edit',
      }),
    ).toBe('spliced');
    expect(await readFile(join(root, 'a.txt'), 'utf-8')).toBe(
      `line 1\nline 2 REPLACED\nline 3\nline 4\n`,
    );
  });
