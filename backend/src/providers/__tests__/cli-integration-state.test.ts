...
describe('native CLI integration state', () => {
  it('increments the rewrite revision used to invalidate native conversations', () => {
    resetCliConversationRevisions();
    expect(getCliConversationRevision('session-a')).toBe(0);
    expect(markCliConversationRewritten('session-a')).toBe(1);
    expect(getCliConversationRevision('session-a')).toBe(1);
    expect(getCliConversationRevision('session-b')).toBe(0);
  });
