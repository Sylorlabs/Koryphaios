...
function buildModelFromId(modelId: string, displayName?: string): ModelDef {
  ...
  return {
    ...
    // cursor-agent exposes model selection only. Names such as "auto" or
    // "thinking" are model names, not evidence of a separate reasoning-effort
    // flag. Keep the capability explicit so the UI never invents tiers.
    canReason: false,
    reasoningLevels: [],
  } as ModelDef;
}
