...
export class GrokBuildProvider implements Provider {
  ...
  async *streamResponse(request: StreamRequest): AsyncGenerator<ProviderEvent> {
    ...
    const args = [
      ...
      '--no-subagents',
      ...(researchOnly
        ? ['--tools', 'web_search,web_fetch', '--max-turns', '4', '--no-memory']
        : []),
    ...
    try {
      ...
      providerLog.debug(
        ...
        'grok-build: agent settings unavailable, keeping web search on',
      );
    }
    ...
    const baseEnv = buildProviderCliEnv('grok', {
      GROK_HOME: grokHome,
      HOME: grokHome,
    ...
    const grokEnv = { ...(jail?.env ?? baseEnv) };
    ...
    const onAbort = () => {
      try {
        ...
        providerLog.debug(
          { err: err instanceof Error ? err.message : String(err) },
          'grok-build: harness child already gone on abort',
        );
      }
    };
...
function readGrokCliModelsCache(): Map<string, GrokCliModelMeta> | null {
  try {
    ...
    providerLog.debug(
      { err: err instanceof Error ? err.message : String(err) },
      'grok-build: failed to read CLI models cache',
    );
    return null;
  }
}
...
export function parseGrokCliModelsCache(raw: string): Map<string, GrokCliModelMeta> | null {
  try {
    ...
    providerLog.debug(
      { err: err instanceof Error ? err.message : String(err) },
      'grok-build: failed to parse CLI models cache JSON',
    );
    return null;
  }
}
...
async function probeGrokReasoningLevels(bin: string): Promise<string[] | null> {
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(bin, ['--reasoning-effort', '__koryphaios_probe__', '-p', ''], {
      ...
      env: buildProviderCliEnv('grok', {
        GROK_HOME: getKoryphaiosGrokHome(),
        HOME: getKoryphaiosGrokHome(),
        USERPROFILE: getKoryphaiosGrokHome(),
      }),
    });
    let err = '';
    ...
    setTimeout(() => {
      try {
        ...
        providerLog.debug(
          { err: err instanceof Error ? err.message : String(err) },
          'grok-build: effort probe child already gone on timeout kill',
        );
      }
      done(null);
    }, 8_000);
...
export function parseGrokOutput(raw: string): {
  ...
  try {
    ...
    providerLog.debug(
      { err: err instanceof Error ? err.message : String(err) },
      'grok-build: output is not a single JSON object, trying NDJSON',
    );
  }
  ...
  if (lines.some((l) => l.startsWith('{'))) {
    ...
    for (const line of lines) {
      try {
        ...
        providerLog.debug(
          { err: err instanceof Error ? err.message : String(err) },
          'grok-build: skipping non-JSON NDJSON line',
        );
      }
    }
...
function drainGrokToolTailer(state: GrokToolTailer): ProviderEvent[] {
  ...
  try {
    ...
    providerLog.debug(
      { err: err instanceof Error ? err.message : String(err) },
      'grok-build: tool tailer failed to read updates file',
    );
    return events;
  }
  ...
  for (const line of fresh.slice(0, lastNl).split('\n')) {
    ...
    try {
      ...
      providerLog.debug(
        { err: err instanceof Error ? err.message : String(err) },
        'grok-build: tool tailer skipping non-JSON line',
      );
      continue;
    }
