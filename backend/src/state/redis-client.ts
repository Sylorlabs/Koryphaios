...
// Refactored to use the main Redis client from redis/client.ts

import {
  getRedisClient,
  initializeRedis,
  closeRedis as closeMainRedis,
  type RedisConfig as MainRedisConfig,
} from "../redis/client";
 ...
 */
export async function initRedis(config: RedisConfig = { enabled: false }): Promise<void> {
  if (!config.enabled) {
  ...
  try {
    const mainConfig: MainRedisConfig = {
      host: config.host,
      port: config.port,
      password: config.password,
      db: config.db,
      fallbackToMemory: true, // Always allow fallback for state
    };

    await initializeRedis(mainConfig);
    redisEnabled = true;
    serverLog.info("Redis initialized for distributed state management");
  } catch (err) {
    serverLog.error({ err }, "Failed to initialize Redis for state management");
    redisEnabled = false;
  }
}
    ...
    try {
      ...
      for (const [key, value] of Object.entries(hash)) {
        try {
          ...
          serverLog.debug({ key, value: String(value).slice(0, 100), error: err instanceof Error ? err.message : String(err) }, "Session state value is not valid JSON, using raw value");
          result[key] = value;
        }
      }
      return result as T;
