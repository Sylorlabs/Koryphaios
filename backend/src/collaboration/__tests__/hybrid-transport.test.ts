...
beforeAll(async () => {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    try { if ((await fetch(`${relayUrl}/health`)).ok) return; } catch (err: unknown) { serverLog.debug({ err: err instanceof Error ? err.message : String(err) }, 'Relay not yet up; retrying'); }
    await Bun.sleep(25);
  }
