...
beforeAll(async () => {
  ...
  relay = Bun.spawn(['bun', 'run', resolve(import.meta.dir, '../../../../relay/server.ts')], {
    env: {
      ...
      JWT_SECRET: 'hybrid-test-jwt-secret',
    },
    stdout: 'ignore',
    stderr: 'ignore',
  });
...
describe.skipIf(!LOOPBACK_AVAILABLE)('WebRTC-first collaboration transport', () => {
  test('uses a direct data channel and retains the relay as signaling/fallback', async () => {
    ...
    await host.updatePolicy({
      ...structuredClone(DEFAULT_COLLABORATION_POLICY),
      joinMode: 'auto',
      defaultTierId: 'collaborator',
    });
    ...
    const guestWs = new WebSocket(
      `ws://127.0.0.1:${port}/ws?token=${encodeURIComponent(token!)}&name=HybridTest`,
    );
    ...
    channel.stateChanged.subscribe((state) => {
      if (state === 'open') directOpen = true;
    });
    channel.onMessage.subscribe((raw) => {
      directHostEvent = String(raw);
    });
    guestPeer.onIceCandidate.subscribe((candidate) => {
      if (candidate && guestWs.readyState === WebSocket.OPEN)
        guestWs.send(JSON.stringify({ type: 'rtc-ice', candidate: candidate.toJSON() }));
    });
    host.onMessage((message) => {
      if (message.type === 'guest-prompt') receivedPrompt = message;
    });
    ...
    channel.send(
      JSON.stringify({ type: 'guest-prompt', name: 'HybridTest', content: 'direct hello' }),
    );
