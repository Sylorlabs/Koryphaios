...
| `KORYPHAIOS_PORT` | Bind port | `3000` |
...
| Variable | Purpose | Default |
|----------|---------|---------|
| `KORYPHAIOS_KMS_PROVIDER` | KMS backend: `local`, `aws-kms`, `vault`, `gcp-kms`, `azure-kv`, `cloudflare` | `local` |
...
- **Linux**: uses `bwrap` (bubblewrap) for mount-namespace + network
  ...
  `dnf install bubblewrap` (Fedora). If `bwrap` is not on PATH, falls
  back to argv-only execution with a warning.
- **macOS**: uses `sandbox-exec` with a generated profile that allows
  the workspace root and denies network when requested.
- **Windows / unsupported**: falls back to argv-only execution (no
  shell interprets the command string).

When the OS sandbox is off (default), sandboxed commands still run
argv-only (no `bash -c <string>`), which closes the regex-bypass hole.
The OS sandbox adds kernel-level confinement on top.
