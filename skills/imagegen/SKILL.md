...
metadata:
  koryphaios:
    ...
    requires: []
    conflicts: []
    activation: ["generate an image", "create a picture", "image generation", "edit this image", "transparent background", "product shot"]
    excludes: ["SVG", "vector icon", "diagram", "wireframe"]
    domains: ["image-generation", "visual-assets"]
    targetMedia: ["any"]
    shouldTrigger: ["generate a hero image for my landing page", "create a product shot with transparent background", "edit this photo to remove the background"]
    shouldNotTrigger: ["create an SVG icon", "build a bar chart in CSS", "fix a CSS bug"]
    evidence: ["Generated image file and validation"]
    contextBudget: 8000
...
```
Need an image?
├─ Is it a new image or an edit of an existing one?
│  ├─ New image → built-in image_gen (generate)
│  └─ Edit existing image
│     ├─ Image already in conversation context → built-in image_gen (edit)
│     └─ Image only on local filesystem
│        ├─ Load with view_image first → built-in image_gen (edit)
│        └─ Needs masks or CLI-only params → CLI fallback (only if user asks)
│
├─ Need transparent background?
│  ├─ Simple opaque subject → built-in image_gen + chroma-key removal
│  └─ Complex subject (hair, fur, glass) → ask user before CLI gpt-image-1.5
│
├─ Need many assets?
│  ├─ Built-in mode → one image_gen call per asset
│  └─ CLI mode (user explicitly chose) → generate-batch subcommand
│
└─ User explicitly asks for CLI/API/model path?
   └─ CLI fallback with scripts/image_gen.py
```
...
|-------------|---------------|-------------------|
| Silently switching to CLI fallback | Downgrades model/path without user consent | Stay on built-in `image_gen` unless user explicitly asks for CLI |
| Using `n` for distinct assets | `n` produces variants of one prompt, not different images | Issue separate `image_gen` calls per distinct asset |
| Leaving project assets at `$CODEX_HOME/*` | Project-referenced assets must live in the workspace | Move or copy final selected image into the project |
| Overwriting existing assets without asking | Destroys previous work | Create sibling versioned filename (`hero-v2.png`) |
| Using SVG/HTML for photorealistic requests | Wrong tool for the job | Use `image_gen` for photos, illustrations, product shots |
| Adding creative details the user didn't ask for | Changes the spec without consent | Only augment generic prompts; preserve specific ones |
| Skipping chroma-key validation | Transparent output may have fringe or holes | Validate alpha channel, transparent corners, subject coverage |
| Setting `input_fidelity` with `gpt-image-2` | Not supported by this model | Only set `input_fidelity` with `gpt-image-1.5` |
| Asking user to paste `OPENAI_API_KEY` in chat | Security risk | Ask them to set it as env var and confirm when ready |
| Modifying `scripts/image_gen.py` | Breaks the bundled CLI tool | Ask the user before changing anything in the script |
