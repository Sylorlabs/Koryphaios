...
metadata:
  koryphaios:
    ...
    requires: []
    conflicts: []
    activation: ["create a plugin", "make a plugin", "scaffold a plugin", "marketplace entry", "plugin manifest"]
    excludes: []
    domains: ["plugins", "skills"]
    targetMedia: ["any"]
    shouldTrigger: ["create a new personal plugin", "add a marketplace entry for my plugin", "scaffold a codex plugin"]
    shouldNotTrigger: ["install a skill", "fix a CSS bug"]
    evidence: ["Plugin manifest validation and marketplace entry"]
    contextBudget: 6000
...
|-------------|---------------|-------------------|
| Hand-editing `marketplace.json` | Breaks the scaffold's managed fields | Use `scripts/update_plugin_cachebuster.py` |
| Leaving `[TODO: ...]` in manifest | Validation rejects it | Replace all TODOs with real values before validating |
| Adding `hooks` to `plugin.json` | Validation rejects unsupported fields | Hooks go in a separate file referenced by the `hooks` path |
| Adding `apps`/`mcpServers` without companion files | Manifest references non-existent files | Create the companion file first, or remove the field |
| Folder name ≠ `plugin.json` name | Ingestion path expects them to match | Both must use the same normalized name |
| Non-https URLs in policy/website fields | Validation requires absolute https URLs | Use `https://` absolute URLs |
| More than 3 `defaultPrompt` entries | Extras are silently ignored | Keep at most 3, each ≤128 chars |
| Running `codex plugin marketplace add` for personal marketplace | Personal marketplace is discovered implicitly | Only use that command for explicit non-default marketplaces |
| Using `--marketplace-name` to rename existing file | That flag is for seeding NEW marketplace files | Point to the existing file with `--marketplace-path` |
| Not verifying non-default marketplace is installed | User can't reinstall from unconfigured marketplace | Run `codex plugin marketplace add <path>` first |

For the full list with detailed explanations and edge cases, see `references/common-mistakes.md`.

## Reference to exact spec sample

For the exact canonical sample JSON for both plugin manifests and marketplace entries, use:

- `references/plugin-json-spec.md` — Full manifest and marketplace JSON specs with field guides
- `references/installing-and-updating.md` — Update/reinstall guidance while iterating on an existing local plugin, plus new-thread pickup behavior after reinstall
- `references/common-mistakes.md` — Common manifest and marketplace mistakes, edge cases, name normalization rules, deeplink format
