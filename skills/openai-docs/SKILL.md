...
metadata:
  koryphaios:
    ...
    requires: []
    conflicts: []
    activation: ["openai docs", "how to use the API", "which model", "latest model", "codex documentation", "model migration", "prompting guide"]
    excludes: []
    domains: ["documentation", "openai", "models"]
    targetMedia: ["any"]
    shouldTrigger: ["how do I use the responses API", "which model should I use for my use case", "how to migrate from gpt-4 to the latest model"]
    shouldNotTrigger: ["fix a CSS bug", "deploy to AWS"]
    evidence: ["Cited documentation from official OpenAI sources"]
    contextBudget: 6000
...
```
User request?
├─ Codex self-knowledge (config, skills, plugins, MCP, hooks, surfaces)?
│  └─ Codex source route → manual helper first, then Docs MCP, then bounded uncertainty
│
├─ Latest/current/default model selection or prompting guidance?
│  ├─ Pure model-selection question (no change requested)
│  │  └─ Fetch latest-model.md directly
│  └─ Prompting guidance or change requested + latest/current target
│     └─ Run resolver script first → fetch returned guide URLs
│
├─ Explicit named model target (e.g., "migrate to GPT-5.4")?
│  └─ Preserve target; fetch model-guidance page for that model
│
├─ General OpenAI docs question (APIs, SDKs, products)?
│  └─ Docs MCP: search → fetch → answer with citations
│
├─ API reference / schema / parameter question?
│  └─ get_openapi_spec + relevant guide page
│
└─ Docs MCP unavailable or unhelpful?
   └─ Web search restricted to official OpenAI domains
```

## Source Priority Table

| Request type | Primary source | Fallback | Notes |
|-------------|---------------|----------|-------|
| Codex self-knowledge | Manual helper (`scripts/fetch-codex-manual.mjs`) | Docs MCP → bounded uncertainty → official web | Manual is first for broad Codex synthesis; Docs MCP is a different lane, not interchangeable |
| Latest/current prompting guidance | Resolver script → returned guide URLs | Bundled `references/prompting-guide.md` | Run resolver before any direct fetch or API-key gate |
| Pure model-selection question (no change) | `https://developers.openai.com/api/docs/guides/latest-model.md` | `references/latest-model.md` | Don't run resolver for pure selection questions |
| Explicit named model target | Model-guidance page for that model | Bundled references | Preserve user's target even if newer model exists |
| General OpenAI docs | `search_openai_docs` → `fetch_openai_doc` | Official-domain web search | Use compact 2-6 term queries, not full questions |
| API reference / schema / parameters | `get_openapi_spec` | Relevant guide page via Docs MCP | Verify API shape alongside guides |
| Browse/discover without clear query | `list_openai_docs` | Docs MCP search | Only when you need to discover pages |
| Model upgrade / prompt upgrade | Resolver → migration + prompting guide URLs | `references/upgrade-guide.md` | Keep changes behavior-preserving and scoped |
...
### Codex Source Route Quick Reference

| Step | When | Action |
|------|------|--------|
| 1. Reuse same-thread manual | Manual path is fresh and from this thread | Read from existing path; skip re-fetching |
| 2. Run helper | Normal writable session, no fresh manual | `node <skill-dir>/scripts/fetch-codex-manual.mjs` |
| 3. Skip to Docs MCP | Read-only session, no shell, or no temp cache allowed | `search_openai_docs` → `fetch_openai_doc` |
| 4. Gap-fill with Docs MCP | Manual missing a material claim or term | One narrow search + one fetch |
| 5. Bounded uncertainty | Manual + Docs MCP don't establish the claim | State uncertainty; don't widen investigation |
| 6. Official web fallback | Docs MCP unavailable or unhelpful | Web search restricted to openai.com domains |
| 7. Stop | Claim still not established | Return bounded uncertainty or route to support |

Refresh the manual when: fetched >1 day ago, path unusable, path from another thread, or staleness is plausible.
